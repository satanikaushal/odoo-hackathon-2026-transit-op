...
3. DB: first tried `bunx prisma dev --detach` (Prisma's embedded PGlite Postgres, no Docker needed) since no Docker daemon was reachable at the time. Got real migrations working on it (`prisma/migrations/20260712051431_init` baseline via `prisma migrate diff` + `prisma migrate resolve --applied`, then normal `migrate dev` after that) — but PGlite's session handling turned out not to be reliable under concurrent connections: a `prisma migrate reset` corrupted its prepared-statement state (`ERROR: prepared statement "s3" already exists`) badly enough that even `db push` on a fresh dev-server restart couldn't recover it. **Moved to a real Postgres 16 in Docker** (`docker-compose.yml` + `docker/init-shadow-db.sql`, which creates both `transitops` and `transitops_shadow` on first boot) — no further issues. `bun run db:migrate`/`db:deploy`/`db:push` all work normally against it.
4. `src/lib/prisma.ts` — singleton `PrismaClient` wired with `PrismaPg` adapter (avoids multiple instances under `--hot`).
5. `src/config/env.ts` extended with `DATABASE_URL`, `JWT_ACCESS_SECRET`, token TTLs (fails fast if missing). No `JWT_REFRESH_SECRET` — refresh tokens are opaque random values, not JWTs, so there's nothing to sign (see `docs/auth.md`).
6. `prisma/seed.ts`: one user per role (`ADMIN`, `FLEET_MANAGER`, `DRIVER`, `SAFETY_OFFICER`, `FINANCIAL_ANALYST`), each with a freshly generated random password (`Bun.password.hash`) printed once to the console — never hardcoded, never stored in plaintext. Reruns skip existing users rather than resetting their password. Run with `bun run db:seed`.
7. `package.json` scripts: `db:migrate`, `db:deploy`, `db:push`, `db:generate`, `db:seed`, `db:studio`. See `README.md` for the day-to-day DB commands.
...
2. ✅ `src/services/auth.service.ts` — verifies password (`Bun.password.verify`), issues access JWT (`src/lib/jwt.ts`, 15m default) + opaque refresh token (`src/lib/refreshToken.ts`, sha256-hashed before storing in `RefreshToken`). Refresh **rotates**: each use revokes the old token row and issues a new pair; reuse of a spent/revoked/expired token is rejected. Login/refresh responses include `accessTokenExpiresAt`/`refreshTokenExpiresAt`. `RefreshToken` also optionally carries `deviceType`/`deviceToken` (FCM push token) from login, carried forward across rotation — see `docs/auth.md` § Device tokens (storage only, no push sending wired up yet).
